...
class DevTools(commands.Cog):
    ...
    async def cog_load(self):
        ...
        self.github = await github.GHClient(username="JDJGBot",
                                            token=github_token)
        self.rtfm_dictionary = sorted(
            await self.bot.db.fetch("SELECT * FROM RTFM_DICTIONARY"))
        ...
        self.invalidation_config = [
            utils.InvalidationConfig(record.entity_id, record.entity_type,
                                     self.bot) for record in await
            self.bot.db.fetch("SELECT * FROM invalidation_config")
        ]
        self.invalidation_opt_out = [
            utils.InvalidationConfig(record.entity_id, record.entity_type,
                                     self.bot) for record in await
            self.bot.db.fetch("SELECT * FROM invalidation_out")
        ]
    ...
    async def token_snipper(self, ctx):
        embed = discord.Embed(
            ...
            description=
            "It tells the bot if it should invalidate any discord tokens sent into chat",
        ...
        await ctx.send(
            "Going to change to a slash command with explicit permissions for some options."
        )
    ...
    @commands.command(
        ...
        brief=
        "A RTFM command that allows you to lookup any supported library (using selects)",
    )
    ...
    async def rtfm_slash(self,
                         interaction: discord.Interaction,
                         library: typing.Optional[str] = None,
                         query: typing.Optional[str] = None) -> None:
    ...
    async def rtfm_library_autocomplete(
        ...
        libraries = dict(self.rtfm_dictionary)
        all_choices = [
            app_commands.Choice(name=name, value=link)
            for name, link in libraries.items()
        ]
        startswith = [
            choice for choice in all_choices
            if choice.name.lower().startswith(current.lower())
        ]
    ...
    async def rtfm_query_autocomplete(
        ...
        unfiltered_results = await utils.rtfm(self.bot, url)
        all_choices = [
            app_commands.Choice(name=result.name,
                                value=result.url.replace(url, ""))
        ...
        filtered_results = fuzzy.finder(current,
                                        unfiltered_results,
                                        key=lambda t: t.name)
        return [
            app_commands.Choice(name=result.name,
    ...
    @rtfm_slash.error
    async def rtfm_error(self, interaction: discord.Interaction,
                         error: Exception) -> None:
        await interaction.response.send_message(
            f"An error occurred: {error}. Please report this to my developer.",
            ephemeral=True)
    ...
    @commands.command(
        brief="Gives you data about characters (based on R.danny's command)")
    async def charinfo(self, ctx, *, characters: str):
        if not characters:
            return await ctx.send(
                "Please provide characters to get information about.")
    ...
    async def pep8(self, ctx):
        ...
        if not modal.value:
            return await ctx.reply("You need to provide code to format.",
        ...
        await message.edit(
            content=("Using special formatting (120 characters)" if
                     use_special_formatting else "Using default formatting"),
            view=None,
        )

        try:
            formatted_code = await asyncio.to_thread(utils.formatter,
                                                     code.content,
                                                     use_special_formatting)
        ...
        embed.set_footer(
            text=
            "Make sure you use Python code, otherwise it may not work properly."
        )
    ...
    async def pfp_grab(self, ctx):
        ...
        try:
            ...
            await ctx.send(
                "Unable to send the profile picture due to file size limitations."
            )
    ...
    async def pypi(self, ctx, *, package_name: str = None):
        if not package_name:
            return await ctx.send(
                "Please provide a package name to get information.")

        async with self.bot.session.get(
                f"https://pypi.org/pypi/{package_name}/json") as response:
            if not response.ok:
                return await ctx.send(
        ...
        embed = discord.Embed(
            title=
            f"{pypi_data.get('name', 'N/A')} {pypi_data.get('version', 'N/A')}",
    ...
    async def invite_bot(self, ctx, *, user: discord.User = commands.Author):
        ...
        view.add_item(
            discord.ui.Button(label=f"{user.name}'s Normal Invite",
                              url=invite,
                              style=discord.ButtonStyle.link))
        view.add_item(
            discord.ui.Button(
                label=f"{user.name}'s Invite With Slash Commands",
                url=slash_invite,
                style=discord.ButtonStyle.link))

        await ctx.send(
            "Invite with slash commands and the bot scope or only with a bot scope:",
            view=view)

    @commands.command(brief="Puts the message time as a timestamp")
    async def message_time(self, ctx):
        embed = discord.Embed(title="Message Time",
                              color=random.randint(0, 16777215),
                              timestamp=ctx.message.created_at)
    ...
    async def created_at(self,
                         ctx,
                         *,
                         user: utils.SuperConverter = commands.Author):
    ...
    async def snowflake_info(self,
                             ctx,
                             *,
                             snowflake: typing.Optional[
                                 utils.ObjectPlus] = None):
        if not snowflake:
            snowflake = await utils.ObjectPlusConverter().convert(
                ctx, argument=str(utils.generate_snowflake()))
        ...
        embed.add_field(
            name="Created At:",
            value=
            f"{discord.utils.format_dt(snowflake.created_at, style='D')}\n{discord.utils.format_dt(snowflake.created_at, style='T')}",
        )
    ...
    async def fake_token(self, ctx):
        ...
        first_bit = base64.b64encode(str(
            discord_object.id).encode()).decode().rstrip("=")
        timestamp = int(discord_object.created_at.timestamp() - 129384000)
        second_bit = base64.standard_b64encode(timestamp.to_bytes(
            4, "big")).decode().rstrip("=")
    ...
    async def addbot(self, ctx, *, user: discord.User = commands.Author):
        ...
        message = await ctx.send(
            "Please tell us the reason you want to add your bot to the Test Guild:",
            view=modal)
        await modal.wait()
        ...
        if modal.value is None:
            ...
            return await message.edit(
                content=
                "Please provide a reason why you want your bot added to the guild."
            )
        ...
        if member is None:
            view = discord.ui.View()
            view.add_item(
                discord.ui.Button(label="Test Guild Invite",
                                  url="https://discord.gg/hKn8qgCDzK",
                                  style=discord.ButtonStyle.link))
            return await message.edit(
                content=
                "Make sure to join the guild linked soon... then rerun the command. If you are in the guild, contact the owner (listed in the owner command).",
                view=view,
            )
        ...
        await self.bot.get_channel(852897595869233182).send(
            content=jdjg.mention, embed=embed)
    ...
    @commands.command(
        brief="Checks if a URL is a valid image (requires embed permissions)")
    async def image_check(self, ctx):
        ...
        if not images:
            return await ctx.send(
                "You need to provide a URL with an image. If you did, please try again."
            )

        await ctx.send(
            f"You have {len(images)} valid image link(s) out of {len(ctx.message.embeds)} total link(s)."
        )

    @commands.command(brief="Gives info on npm packages")
    async def npm(self, ctx, *, args=None):
        if args:
            npm_response = await self.bot.session.get(
                f"https://registry.npmjs.com/{args}")
            ...
            if npm_response.ok:
                ...
                await ctx.send(f"Could not find package **{args}** on npm.",
                               allowed_mentions=discord.AllowedMentions.none())
    ...
    @commands.command(
        brief="runs some code in a sandbox(based on Soos's Run command)",
        aliases=["eval", "run", "sandbox"])
    async def console(self, ctx, *, code: codeblock_converter = None):
        if not code:
            ctx.command.reset_cooldown(ctx)
            return await ctx.send(
                "You need to give me some code to use, otherwise I can not determine what it is."
            )
        ...
        output = await self.tio.execute(f"{code.content}",
                                        language=f"{code.language}")

        text_returned = (f"```{code.language}\n{output}```" if len(f"{output}")
                         < 200 else await utils.post(self.bot,
                                                     code=f"{output}"))

        embed = discord.Embed(
            title=f"Your code exited with code {output.exit_status}",
            description=f"{text_returned}",
            color=242424)
        ...
        await ctx.send(content="I executed your code in a sandbox",
                       embed=embed)
